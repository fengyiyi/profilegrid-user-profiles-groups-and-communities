...
else
{
	...
	if(!empty($row))
	{
		if($row->field_options!="")$field_options = maybe_unserialize($row->field_options);
...
if(filter_input(INPUT_POST,'submit_field'))
{
        ...
        {
            echo '<div class="error">'.__('you have already created a user email field.',$textdomain).'</div>';
        }
        else
        {
	$retrieved_nonce = filter_input(INPUT_POST,'_wpnonce');
	if (!wp_verify_nonce($retrieved_nonce, 'save_pm_add_field' ) ) die( __('Failed security check',$textdomain) );
	...
	$post = $pmrequests->sanitize_request($_POST,$identifier,$exclude);
        ...
        if($field_type=='user_email')$post['show_in_signup_form']=1;
	if(!isset($post['show_in_signup_form'])) $post['show_in_signup_form'] = 0;
	if(!isset($post['is_required'])) $post['is_required'] = 0;
	if(!isset($post['is_editable'])) $post['is_editable'] = 0;
	if(!isset($post['display_on_profile'])) $post['display_on_profile'] = 0;
	if(!isset($post['display_on_group'])) $post['display_on_group'] = 0;
	if($post!=false)
	{
	...
	if($data['field_key']=='')
	{
		if($fieldid==0){$field_key_id = $data['ordering'];}else{$field_key_id = $fieldid;}
  ...
  else
  {
	...
	wp_redirect('admin.php?page=pm_profile_fields&gid='.$gid.'#'.sanitize_key($sectionname));exit;
        }
}
...
if(filter_input(INPUT_POST,'delete'))
{
	...
	wp_redirect('admin.php?page=pm_profile_fields&gid='.$gid);exit;
}
...
<div class="uimagic">
  <form name="pm_add_group" id="pm_add_field" method="post">
    ...
    <div class="content">
      <?php if($id==0): ?>
      <div class="uimheader">
        <?php _e( 'New Field', $textdomain ); ?>
      </div>
      <?php else: ?>
      <div class="uimheader">
        <?php _e( 'Edit Field', $textdomain ); ?>
      </div>
      <?php endif; ?>
      ...
      <div class="uimrow">
        ...
        <div class="uimnote"><?php _e('Label of the field as it appears on forms and profiles. This does not apply to fields without labels such as Heading, Paragraph, Divider, Spacing.',$textdomain);?></div>
      </div>
      <div class="uimrow">
        <div class="uimfield">
          <?php _e( 'Field Description:', $textdomain ); ?>
        </div>
        <div class="uiminput">
          <textarea name="field_desc" id="field_desc"><?php if(!empty($row))echo esc_attr($row->field_desc); ?>
</textarea>
        </div>
        <div class="uimnote"><?php _e('For your reference only. Not visible on front-end. Description can help you remember the purpose of the field.',$textdomain);?> </div>
      </div>
      
      <div class="uimrow">
        <div class="uimfield">
          <?php _e( 'Field Type', $textdomain ); ?>
        </div>
        <div class="uiminput">
          ...
          <option value="term_checkbox" <?php if(isset($str) && $str=='term_checkbox') echo 'selected'; ?>><?php _e( 'T&C Checkbox', $textdomain ); ?></option>
			<option value="file" <?php if(isset($str) && $str=='file') echo 'selected'; ?>><?php _e( 'File Upload', $textdomain ); ?></option>
           <!-- <option value="pricing" <?php if(isset($str) && $str=='pricing') echo 'selected'; ?>><?php _e( 'Pricing', $textdomain ); ?></option> -->
            <option value="repeatable_text" <?php if(isset($str) && $str=='repeatable_text') echo 'selected'; ?>><?php _e( 'Repeatable Text', $textdomain ); ?></option>
            ...
            <option value="user_name" <?php if(isset($str) && $str=='user_name') echo 'selected'; ?>><?php _e( 'User Name', $textdomain ); ?></option>
            <?php  
    $check = $pmrequests->pm_check_field_exist($gid,'user_email',true);
    if($check===false || $str=='user_email' ):
    ?>
            <option value="user_email" <?php if(isset($str) && $str=='user_email') echo 'selected'; ?>><?php _e( 'User Email', $textdomain ); ?></option>
            <?php endif;?> 
            <option value="user_url" <?php if(isset($str) && $str=='user_url') echo 'selected'; ?>><?php _e( 'Website', $textdomain ); ?></option>
            <option value="user_pass" <?php if(isset($str) && $str=='user_pass') echo 'selected'; ?>><?php _e( 'Password', $textdomain ); ?></option>
            <option value="confirm_pass" <?php if(isset($str) && $str=='confirm_pass') echo 'selected'; ?>><?php _e( 'Confirm Password', $textdomain ); ?></option>
            <option value="description" <?php if(isset($str) && $str=='description') echo 'selected'; ?>><?php _e('Biographical Info', $textdomain ); ?></option>
            <option value="user_avatar" <?php if(isset($str) && $str=='user_avatar') echo 'selected'; ?>><?php _e( 'Profile Image', $textdomain ); ?></option>
            <option value="mobile_number" <?php if(isset($str) && $str=='mobile_number') echo 'selected'; ?>><?php _e( 'Mobile Number', $textdomain ); ?></option>
            <option value="phone_number" <?php if(isset($str) && $str=='phone_number') echo 'selected'; ?>><?php _e( 'Phone Number', $textdomain ); ?></option>
            <option value="gender" <?php if(isset($str) && $str=='gender') echo 'selected'; ?>><?php _e( 'Gender', $textdomain ); ?></option>
            <option value="language" <?php if(isset($str) && $str=='language') echo 'selected'; ?>><?php _e( 'Language', $textdomain ); ?></option>
            <!--<option value="birth_date" <?php if(isset($str) && $str=='birth_date') echo 'selected'; ?>><?php _e( 'Birth Date', $textdomain ); ?></option> -->
            <option value="divider" <?php if(isset($str) && $str=='divider') echo 'selected'; ?>><?php _e( 'Divider', $textdomain ); ?></option>
            <option value="spacing" <?php if(isset($str) && $str=='spacing') echo 'selected'; ?>><?php _e( 'Spacing', $textdomain ); ?></option>
            <option value="multi_dropdown" <?php if(isset($str) && $str=='multi_dropdown') echo 'selected'; ?>><?php _e( 'Multi Dropdown', $textdomain ); ?></option>
            <option value="facebook" <?php if(isset($str) && $str=='facebook') echo 'selected'; ?>><?php _e( 'Facebook', $textdomain ); ?></option>
            <option value="twitter" <?php if(isset($str) && $str=='twitter') echo 'selected'; ?>><?php _e( 'Twitter', $textdomain ); ?></option>
            <option value="google" <?php if(isset($str) && $str=='google') echo 'selected'; ?>><?php _e( 'Google+', $textdomain ); ?></option>
            <option value="linked_in" <?php if(isset($str) && $str=='linked_in') echo 'selected'; ?>><?php _e( 'Linked In', $textdomain ); ?></option>
            <option value="youtube" <?php if(isset($str) && $str=='youtube') echo 'selected'; ?>><?php _e( 'Youtube', $textdomain ); ?></option>
            <option value="instagram" <?php if(isset($str) && $str=='instagram') echo 'selected'; ?>><?php _e( 'Instagram', $textdomain ); ?></option>
            <!--<option value="address" <?php if(isset($str) && $str=='address') echo 'selected'; ?>><?php _e( 'Address', $textdomain ); ?></option> -->
       
          </select>
        </div>
        <div class="uimnote"><?php _e('Change the type of this field. Please note, changing a type of an existing field with values may result in data loss. Use this carefully.',$textdomain);?></div>
      </div>
      
       <div class="childfieldsrow" id="field_options_wrapper" style=" <?php if(isset($str)){echo 'display:block;';} else { echo 'display:none;';} ?>">
       
       <div class="uimrow" id="place_holder_text_html">
        <div class="uimfield">
          <?php _e( 'Placeholder text:', $textdomain ); ?>
        </div>
        <div class="uiminput">
        ...
        <div class="uimnote"><?php _e('Placeholder text appears inside input box as guidlines to the user. It disappears when the user clicks on the input box. For e.g. <i>Enter your age</i> inside a number input box.',$textdomain);?></div>
      </div>
      ...
      <div class="uimrow" id="css_class_attribute_html">
        ...
        <div class="uimnote"><?php _e('If you know a bit of CSS, you can create and assign a custom class to this field. Just enter the name of the class as it appears in your stylesheet. For e.g. <i>my-input-class</i>',$textdomain);?></div>
      </div>
      ...
      <div class="uimrow" id="maximum_length_html">
        ...
        <div class="uimnote"><?php _e('Maximum lenght of the allowed field value in characters (count).',$textdomain);?></div>
      </div>
      ...
      <div class="uimrow" id="default_value_html">
        ...
        <div class="uimnote"><?php _e('Default values work for selection boxes where you want a value to be pre-selected when the form loads. You need to enter the value exactly as you have created in the options below.',$textdomain);?> </div>
      </div>
      
      <div class="uimrow" id="first_option_html">
        <div class="uimfield">
          <?php _e( 'First Option Value:', $textdomain ); ?>
        </div>
        <div class="uiminput">
        ...
        <div class="uimnote"><?php _e('Add options for your selection field below. Click on <i>Click to add option</i> to add an extra option to existing ones.',$textdomain);?></div>
      </div>
      ...
      <div class="uimrow" id="field_options_html">
        ...
        <div class="uimnote"><?php _e('Options for drop down list. Separate multiple values with a comma(,).',$textdomain);?></div>
      </div>
      ...
      <div class="uimrow" id="paragraph_text_html">
        <div class="uimfield">
          <?php _e( 'Paragraph Text:', $textdomain ); ?>
        </div>
        <div class="uiminput">
        ...
        <div class="uimnote"><?php _e('The text you want to appear in this field. HTML is supported. Which means you can use HTML tags like <i>b,i, u, em, strong, h1, h2...</i> etc. to style your content.',$textdomain);?> </div>
      </div>
      ...
      <div class="uimrow" id="columns_html">
        ...
        <div class="uimnote"><?php _e('Number of columns (equavalent to character count) allowed in this field. Leave blank for no limit.',$textdomain);?></div>
      </div>
      ...
      <div class="uimrow" id="rows_html">
        ...
        <div class="uimnote"><?php _e('Number of rows (of text) allowed in this field. Leave blank for no limit.',$textdomain);?></div>
      </div>
      
      <div class="uimrow" id="term_and_condition_html">
        <div class="uimfield">
          <?php _e( 'Terms & Conditions:', $textdomain ); ?>
        </div>
        <div class="uiminput">
        ...
        <div class="uimnote"><?php _e('Paste the contents of your Terms and Conditions here. Users will be able to scroll through it and read in full before accepting them.',$textdomain);?></div>
      </div>
      ...
      <div class="uimrow" id="allowed_file_types_html">
        ...
        <div class="uimnote"><?php _e('Separate multiple values by â€œ|â€?. For example PDF|JPEG|XLS',$textdomain);?> </div>
      </div>
      
      <div class="uimrow" id="heading_text_html">
        <div class="uimfield">
          <?php _e( 'Heading Text:', $textdomain ); ?>
        </div>
        <div class="uiminput">
        ...
        <div class="uimnote"><?php _e('Text inside your heading. Remember, headings have large font sizes, therefore a very long text may appear odd. Best used to grab attention for something important. Combine it with <i>Paragraph</i> field type to add extra content.',$textdomain);?></div>
      </div>
      
      <div class="uimrow" id="heading_tag_html">
        <div class="uimfield">
          <?php _e( 'Heading Tag:', $textdomain ); ?>
        </div>
        <div class="uiminput">
        <select name="field_options[heading_tag]" id="heading_tag">
            <option value="h1" <?php if(!empty($field_options))selected($field_options['heading_tag'],'h1');?>><?php _e('Heading 1',$textdomain);?></option>
            <option value="h2" <?php if(!empty($field_options))selected($field_options['heading_tag'],'h2');?>><?php _e('Heading 2',$textdomain);?></option>
            <option value="h3" <?php if(!empty($field_options))selected($field_options['heading_tag'],'h3');?>><?php _e('Heading 3',$textdomain);?></option>
            <option value="h4" <?php if(!empty($field_options))selected($field_options['heading_tag'],'h4');?>><?php _e('Heading 4',$textdomain);?></option>
            <option value="h5" <?php if(!empty($field_options))selected($field_options['heading_tag'],'h5');?>><?php _e('Heading 5',$textdomain);?></option>
            <option value="h6" <?php if(!empty($field_options))selected($field_options['heading_tag'],'h6');?>><?php _e('Heading 6',$textdomain);?></option>
        </select>
        </div>
        <div class="uimnote"><?php _e('Text size decreases from H2 to H6. There maybe additional style applied to the heading text based on your theme CSS.',$textdomain);?></div>
      </div>
      ...
      <div class="uimrow" id="price_html">
        ...
        <div class="uimnote"><?php _e('Price - Obviously, only numbers accepted. A read only field.',$textdomain);?></div>
      </div>
        ...
        <div class="uiminput" id="icon_html">
          ...
          <?php if(!empty($row) && $row->field_icon!=0)
		  {
        ...
        <div class="uimnote"> <?php _e('Jazz up your forms with field icons &#128247;. Icons appear at the beginning of the label. For best results use a square image. For e.g. <i>16px x 16px, 256px x 256px, 512px x 512px</i>...',$textdomain);?></div>
      </div>
      
      <div class="uimrow">
        <div class="uimfield">
          <?php _e( 'Associate with Group', $textdomain ); ?>
        </div>
        ...
        <div class="uimnote"><?php _e('Move this field to a different group. Use carefully, since this will remove the field from current group.',$textdomain);?></div>
      </div>
      
      <div class="uimrow">
        <div class="uimfield">
          <?php _e( 'Associate with Section', $textdomain ); ?>
        </div>
        <div class="uiminput pm_select_required">
        ...
        <div class="uimnote"><?php _e('If you have multiple Profile sections in this group, you can move the field to a different section here. By default, each new field is added to the first section. For e.g. moving a date field labelled <i>Date of Birth</i> from <i>Career</i> section to <i>About</i> section.',$textdomain);?></div>
      </div>
        
      
      <div class="uimrow" id="show_signup">
        <div class="uimfield">
          <?php _e( 'Show in Sign-Up form', $textdomain ); ?>
        </div>
        <div class="uiminput">
          <input name="show_in_signup_form" id="show_in_signup_form" type="checkbox"  class="pm_toggle" value="1" <?php if(!empty($row)){checked($row->show_in_signup_form,1);}else{echo 'checked';}?> style="display:none;" onClick="pm_show_hide(this,'signup_html')" />
          <label for="show_in_signup_form"></label>
        </div>
          <div class="uimnote"><?php _e('The field will appear in the registration form when members sign up for this group. Alternatively, you can hide from the form, and users can then fill it up when editing their profiles later on.',$textdomain);?></div>
      </div>
      <div class="childfieldsrow" id="signup_html" style=" <?php if(!empty($row)){if($row->show_in_signup_form=='1'){echo 'display:block;';} else { echo 'display:none;';}}else{ echo 'display:block;';} ?>">
        <div class="uimrow">
          <div class="uimfield">
            <?php _e( 'Required?', $textdomain ); ?>
          </div>
          <div class="uiminput">
          ...
          <div class="uimnote"> <?php _e('Make this field mandatory. Users will receive an error if they try to submit the form without filling up this field.',$textdomain);?></div>
        </div>
        <?php /*?><div class="uimrow">
          <div class="uimfield">
            <?php _e( 'Can it be edited later?', $textdomain ); ?>
          </div>
          <div class="uiminput">
      ...
      <div class="uimrow" id="displayonprofile">
        <div class="uimfield">
          <?php _e( 'Display on Profile Page', $textdomain ); ?>
        </div>
        <div class="uiminput">
          ...
          <label for="display_on_profile"></label>
                ...
                <div class="uimnote"><?php _e('Show this field on profile page of the members. It is totally possible to show a field in group registration form but hide from the profile page.',$textdomain);?></div>
      </div>
         <div class="childfieldsrow" id="displayprofilehtml" style=" <?php if(!empty($row) && $row->display_on_profile=='1'){echo 'display:block;';} else { echo 'display:none;';} ?>">
      <div class="uimrow">
        <div class="uimfield">
          <?php _e( 'Display on Group Page', $textdomain ); ?>
        </div>
        <div class="uiminput">
        ...
        <div class="uiminput">
          ...
          <div class="uimnote"><?php _e('This will display the field in advance search above the members directory on front end. Selecting it during search will allow users to restrict keyword search to selected field(s).',$textdomain);?></div>
      </div>
        
           <div class="uimrow" id="dateofbirth">
        <div class="uimfield">
          <?php _e( 'range of year dropdown', $textdomain ); ?>
        </div>
               <div class="uiminput">
               ...
               <div class="uimnote"><?php _e('This will Enable this to force selection of date of birth from a certain range.', $textdomain); ?></div>

               

           </div>
         <div class="childfieldsrow" id="dateofbirth_range" style=" <?php if(!empty($row) && !empty($field_options['set_dob_range']) && $field_options['set_dob_range']==1){echo 'display:block;';} else { echo 'display:none;';} ?>">
        <div class="uimrow">
                   <div class="uimrow">
                       <div class="uimfield">
                           <?php _e('Start Year', $textdomain); ?>
                       </div>
                       <div class="uiminput">
                       ...
                       <div class="uimnote"><?php _e('Maximum date of for the field.', $textdomain); ?></div>          
                   </div>
                   <div class="uimrow">
                       <div class="uimfield">
                           <?php _e('End Year', $textdomain); ?>
                       </div>
                       <div class="uiminput">
                       ...
                       <div class="uimnote"><?php _e('Minimum date of for the field.', $textdomain); ?></div>          
                   </div>
               </div>
         </div>
               ...
               <div class="uimrow" id="address_pane">
                   <div class="uimrow">
                       <div class="uimfield">
                           <?php _e('Allow fields', $textdomain); ?>
                       </div>
                       <div class="uiminput">
                            ...
                            <input type="checkbox" name="field_options[state]" value="1" <?php if(!empty($field_options['state']))echo 'checked';?>> City
                            <input type="checkbox" name="field_options[city]" value="1" <?php if(!empty($field_options['city']))echo 'checked';?>> State   
                           <label for="address"></label>
                       </div>
                       <div class="uimnote"><?php _e('Select address fields which you want to show on frontend.', $textdomain); ?></div>          
                   </div>
               </div>
      ...
      <div class="buttonarea"> <a href="<?php echo $cancelurl;?>">
        ...
        <input type="submit" value="<?php _e('Save',$textdomain);?>" name="submit_field" id="submit_field" onClick="return add_field_validation()"  />
