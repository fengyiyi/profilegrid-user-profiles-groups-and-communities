...
class PM_Friends_Functions {
	...
	{	
		...
		$friends = $dbhandler->get_all_result($identifier,'user1,user2', $where,'results',0,false,'id','DESC',$additional);
		$friends_id = array();
		$friends_ids = array();
		if(isset($friends)):
	...
	{	
		...
		$friends = $dbhandler->get_all_result($identifier,'user1,user2', $where,'results',0,false,'id','DESC');
		$friends_id = array();
		$friends_ids = array();
		if(isset($friends)):
		foreach($friends as $friend)
	...
	{	
		...
		foreach($suggested_friends as $suggested)
		{
                    ...
                    if($dbhandler->get_global_option_value('pm_allow_sending_request_to_rejected_person','0')):
                        ...
                        {
                            //print_r($is_removed_from_suggestion->action_date);
                        ...
                        if(!isset($is_removed_from_suggestion))
			 {
				$final_result[] = $suggested; 	 
                         ...
                         //print_r($is_removed_from_suggestion);die;
	...
	public function profile_magic_friends_result_html($users,$uid,$view=1)
	{
		...
		foreach($users as $entry):
			...
			$gid = $pmrequests->profile_magic_get_user_field_value($entry->ID,'pm_group');
            ...
            if($dbhandler->get_global_option_value('pm_enable_private_messaging','1')==1):
                ...
                ?>
                    <div class="pm-friend-message-button">
                      <a id="pm-message-url" class="pm-color" href="<?php echo $messenger_url; ?>" ><?php _e('Message','profile-grid');?></a>
                    </div>
                <?php endif; 
        }
        ...
        {
            ?>
                <div class="pm-friend-message-Remove"><a onclick="pm_unfriend_request('<?php echo $u1;?>','<?php echo $u2;?>',this)" ><?php _e('Remove','profile-grid');?></a></div>
            <?php
        }
            ...
            ?>
               <div class="pm-friend-select">
                          <label class="pm-color"> 
                              <input type="checkbox" name="<?php echo $class.'[]';?>" id="<?php echo $class;?>" class="pm-friends-select-checkbox <?php echo $class;?>" value="<?php echo $u2;?>" onclick="pm_select_friend_checkbox(this)" /> <?php _e('Select','profile-grid');?>
                        </label>
                    </div>
            <?php
        }
        ...
        {
            ?>
                <div class="pm-friend-message-button pm-friend-accept_button"><a onclick="pm_confirm_request('<?php echo $u1;?>','<?php echo $u2;?>',this)" ><?php _e('Accept','profile-grid');?></a></div>
            <?php
        }
        ...
        {
            ?>
                <div class="pm-friend-message-Remove pm-friend-reject_button"><a onclick="pm_reject_friend_request('<?php echo $u1;?>','<?php echo $u2;?>',this)" ><?php _e('Delete','profile-grid');?></a></div>
            <?php
        }
        ...
        {
            ?>
                <div class="pm-friend-message-Remove pm-friend-cancel_button"><a onclick="pm_cancel_request('<?php echo $u1;?>','<?php echo $u2;?>',this)" ><?php _e('Cancel','profile-grid');?></a></div>
            <?php
        }
	...
	public function profile_magic_friend_list_button($user1,$user2,$action=0)
	{
            ...
            if(isset($exist_in_table))
            {
                switch($exist_in_table->status)
                {
                        case 1:
                                ...
                                {
                                    ?>
                                    <span class="pm-color pm_add_friend_request" onclick="pm_cancel_request_rightside('<?php echo $u1;?>','<?php echo $u2;?>',this)"><?php _e('Cancel Request','profile-grid');?></span>
                                    <?php
                                }
                                else
                                {
                                    ?>
                                    <span class="pm-color pm_add_friend_request" onclick="pm_confirm_request_right_side('<?php echo $u1;?>','<?php echo $u2;?>',this)"><?php _e('Accept','profile-grid');?></span>
                                    <span class="pm-color pm_add_friend_request" onclick="pm_reject_friend_request_right_side('<?php echo $u1;?>','<?php echo $u2;?>',this)"><?php _e('Delete','profile-grid');?></span>
                                    <?php
                                }	
                                break;
                        case 2:
                                ?>
                                <span class="pm-color pm_add_friend_request" onclick="pm_unfriend_request_rightside('<?php echo $u1;?>','<?php echo $u2;?>',this)">UnFriend</span>
                                <?php
                                break;
                        case 3:
                                ...
                                if($dbhandler->get_global_option_value('pm_allow_sending_request_to_rejected_person','0')==1):
                                        ...
                                        if($days > 0)
                                        {
                                           ...
                                           // echo $current_date.'</br>';
                                            ...
                                            {
                                                ?>
                                                <span class="pm_add_friend_request" onclick="pm_add_friend_request('<?php echo $u1;?>','<?php echo $u2;?>',this)">Add Friend</span>
                                                <?php
            ...
            else
            {
                ...
                if(is_user_logged_in()):
                    ?>
                                <span class="pm-color pm-add-friend" onclick="pm_add_friend_request('<?php echo $u1;?>','<?php echo $u2;?>',this)">Add Friend</span>
                <?php
                else:
                    ...
                    ?>
                            <span><a href="<?php echo $login_url;?>"><?php echo __("Add Friend","profile-grid");?></a></span>
                    <?php
                endif;
            }
	}
