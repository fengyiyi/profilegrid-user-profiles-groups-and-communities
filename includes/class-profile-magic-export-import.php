...
class PM_Export_Import{
  ...
  public function pm_get_csv_single_user_row($uid,$fids,$sep)
  {
     ...
     if(count($fids)>0)
     {
         ...
         foreach($fids as $fid)
         {
             ...
             $csv_output .='"'. $pmrequests->profile_magic_get_user_field_value($uid,$key).'"'.$sep;
         }
     }
  ...
  public function pm_import_users_from_csv($post)
  {
    ...
    echo '<strong>' . __('Import Process Complete','profile-grid') . '</strong><br />';
    echo '<i class="fa fa-user-plus" aria-hidden="true"></i> ' . __('New Users Created','profile-grid').' - '.$created.'<br/>' ;
    echo '<i class="fa fa-user" aria-hidden="true"></i> ' . __('Existing Users Updated','profile-grid').' - '.$updated.'<br/>' ;
    echo '<i class="fa fa-user-times" aria-hidden="true"></i> ' . __('Total Users Skipped','profile-grid').' - '.$skipped.'<br/></div>' ;
     echo '<b>' . __('Import Details','profile-grid') . '</b><br/>';
  ...
  public function pm_import_user($data,$post,$headings)
  {
    ...
    {
       $log['status'] = 0;
       $log['message'] = 'User is skipped due to empty record in CSV'; 
       return $log;
    }
    //print_r($post);die;
    $dbhandler = new PM_DBhandler;
    ...
    if($userexist)
    {
        ...
        else
        {
            // return log
            $log['status'] = 0;
            $log['message'] = 'There\'s already a user with <b>'.$user_email.' </b>email. So we skipped it.';
            return $log;
        }
    }
    else
    {
        ...
        {
            $log['status'] = 0;
            $log['message'] = 'Invalid Email format for User '.'<b>'.$uemail.'</b>';
            return $log;
        }
        if(is_numeric($user_id)):
            ...
            $log['status'] = 0;
            $log['message'] = 'User not created with following username '.'<b>'.$user_name.'</b>'.' and email '.'<b>'.$user_email.'<b>';
            return $log;
        endif;
    }
  ...
  public function pm_generate_mapping_table($post)
  {
        ...
        <div class="uimrow">
            <div class="uimfield">
                ...
                <div class="uimnote"><?php _e('Step 5: Select a section inside this Group where all imported fields will go. You can later redistribute the fields among different sections in Fields Manager.','profile-grid');?></div> 
        </div>
        ...
        <div class="uimrow">
            <div class="uimfield">
                ...
                <div class="uimnote"><?php _e('Step 6a: Map a column in your CSV to username field of this Group. New users will be assigned these usernames.','profile-grid');?></div> 
        </div>
        ...
        <div class="uimrow">
            <div class="uimfield">
                ...
                <div class="uimnote"><?php _e('Step 6b: Map a column in your CSV to User email field of this Group. Make sure the column contain email IDs.','profile-grid');?></div> 
        </div>
        ...
        foreach($headings as $heading)
        {
            ?>
                ...
                <td><input type="radio" name="pm_map_field_<?php echo $i;?>" value="0" onclick="pm_map_field_show_hide(0,'#pm_map_field_with_<?php echo $i;?>')" checked> Create new field</li></td>
                <td><input type="radio" name="pm_map_field_<?php echo $i;?>" value="1" onclick="pm_map_field_show_hide(1,'#pm_map_field_with_<?php echo $i;?>')"> Map to an existing field</td>
